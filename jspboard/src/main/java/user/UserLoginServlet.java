...
public class UserLoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userID.equals("manager") && userPassword.equals("1234") ) {
			request.getSession().setAttribute("messageType", "관리자 접속 ");
			request.getSession().setAttribute("messageContent", "관리자 페이지로 이동합니다.");
		...
		if(result == 1) {
			...
			request.getSession().setAttribute("messageType", "오류 메시지");
			request.getSession().setAttribute("messageContent", "데이터베이스에 오류가 발생했습니다.");
			response.sendRedirect("login.jsp");
			}
	}
